...
def extract_landmarks(letter):
    ...
    video_files = [os.path.join(path, file) for file in os.listdir(path)]
    ...
    with mp_hands.Hands(static_image_mode=True, max_num_hands=2, min_detection_confidence=0.5) as hands:
        for index, file in enumerate(video_files):
            ...
            if index % 100 == 0:
                logger.info(f" {letter}: working on file no. {index:04}/{video_files_len:04}")

    export_path = os.path.join(OUTPUT_FOLDER, f"{letter}.csv")
    pd.DataFrame(landmarks_export).to_csv(export_path)

    logger.info(f" {letter}: finished {video_files_len:04}/{video_files_len:04}")
